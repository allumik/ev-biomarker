...
stat_comb = (
  ...
  .query("`anova p-val` <= 0.05 and method == 'uf'")
...
# create a dummy dataframe which takes the first celltype in the table and creates coordinates for the overriden y-axis ticks
stat_tmp = pd.DataFrame({
  "base": cycle_order, 
  "celltype": stat_comb["celltype"][0], 
...
base_chart = alt.Chart(stat_comb).encode(
  x=alt.X("celltype:N", sort=stat_comb.celltype.drop_duplicates().values).axis(labelAngle=-45),
...
# create a pseudoaxis plot where we take the first value 
yaxis_group = alt.Chart(stat_tmp).mark_text(clip=False).encode(
...
# create another pseudoaxis plot to show the lineages of celltypes
xaxis_group_colors = alt.Chart(stat_comb).mark_rect().encode(
  x=alt.X("celltype:N", sort=stat_comb.celltype.drop_duplicates().values).axis(labelAngle=-45),
  color=alt.Color("lineage:N")
)

((heatmap + text_overlay_stars + text_overlay_diff + text_overlay_pval + yaxis_group) | xaxis_group_colors).properties(
  width=800,
  height=150
)
