...
```{python}
# load in the exported svg
schema_svg = Path(f"{proj_folder}/manuscript/ev_wf_schema.svg")
if schema_svg.exists():
  study_schema = alt.Chart(data=alt.Data(url=str(schema_svg), format=alt.DataFormat(type="svg")))
  study_schema.show()
else:
  print("Could not find the schema SVG file.")
```
