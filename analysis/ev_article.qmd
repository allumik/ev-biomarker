...
dendro_coord[cols_xk] -= dendro_coord[cols_xk].min(axis=None)
dendro_coord[cols_xk] /= dendro_coord[cols_xk].max(axis=None) # scale the coordinates to 0..1 domain
dendro_coord[cols_yk] *= scaler/dendro_coord[cols_yk].max(axis=None) # scale the y heights
dendro_coord[cols_yk] += 1 # and shift up whole unit
...
```{python}
# the barplot part
dendro_marks = pd.DataFrame({"labels": dend_obj["ivl"], "i": np.linspace(0, 1, len(dend_obj["ivl"]))})
...
barplot = base_bar.mark_bar(width=alt.RelativeBandSize(1)).encode(
...
```{python}
(chart_den + barplot).resolve_scale(x="shared", y="shared").resolve_axis(y="independent")
```