...
biplot, pca = funk.biplot_fractions_altair(
  frac_pred, 
  ccht_uf_pheno, 
...
# get the euclidean distance of explained variance per component
loadings = np.linalg.norm(pca.components_.T * np.sqrt(pca.explained_variance_), axis=1)
loadings_df = pd.DataFrame({
  "Variance Explained": loadings, "Feature": frac_pred.columns
}).sort_values("Variance Explained", ascending=False)  # Reset index for Altair

loadings_plt = alt.Chart(loadings_df).mark_bar().encode(
  x=alt.X("Feature:N", sort=None, axis=alt.Axis(labelAngle=-60)),
  y=alt.Y("Variance Explained:Q"),
  color=alt.Color("Feature:N", legend=None, scale=global_color_scale)
).properties(
...
TODO: Get the DE directionality plot from Elina
...
```{python}
(peruvian_bio_ev | (biplot & loadings_plt)).resolve_scale(
    x='independent',
    y='independent',
    color='independent', # Keep color scale independent - legends are now handled manually
    shape="independent"
  )
# below both of them there should be the directionality plot of DE genes
```
