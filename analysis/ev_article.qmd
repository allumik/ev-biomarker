...
prepped_mat = frac_pred.loc[prepped_pheno.index]
...
corr_mat = (
  ...
  .rename("correlation")
  .to_frame()
  .reset_index(names=["y_vars", "x_vars"])
)
...
base_heatmap = alt.Chart(corr_mat).mark_rect().encode(
    x=alt.X('x_vars:O', sort=corr_mat.x_vars, title=None), # Nominal for categorical x-axis
    y=alt.Y('y_vars:O', sort=corr_mat.y_vars[::-1], title=None), # Nominal for categorical y-axis
    color=alt.Color('correlation:Q', scale=alt.Scale(scheme='magma')) # Quantitative for correlation values, using magma colormap
  ).properties(
    title='Correlation Heatmap of Paired Samples Deconvolution Fractions'
  )

# Create vertical lines layer TODO
x_lines = alt.Chart(
    pd.DataFrame({"x": [corr_mat.x_vars[i-1] for i in np.cumsum(gaps)]})
  ).mark_rule(color="white", strokeWidth=2).encode(x=alt.X("x:O"), xOffset=alt.value(0))
y_lines = alt.Chart(
    pd.DataFrame({"y": [corr_mat.y_vars[i-1] for i in np.cumsum(gaps)]})
  ).mark_rule(color="white", strokeWidth=2).encode(y=alt.Y("y:O"), yOffset=alt.value(0))

# Layer heatmap and lines
final_chart = (base_heatmap + x_lines + y_lines).resolve_scale(
