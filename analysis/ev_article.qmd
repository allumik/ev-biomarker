...
dend_obj = dendrogram(
    linkage_mat,
    labels=ccht_uf_pheno.index,
    no_plot=True
)

dendro_coord = funk.get_df_coord(dend_obj)
dendro_marks = pd.DataFrame(dict(labels=dend_obj["ivl"], i=funk.get_leaf_loc(dend_obj)))

base = alt.Chart(dendro_coord)

# the U shape is composed of a shoulder plus two arms, made with separate lines...
shoulder = base.mark_rule().encode(
    alt.X("xk2:Q", title="").axis(None),
    alt.X2("xk3:Q"),
    alt.Y("yk2:Q", title=""))
...
chart_den = ((shoulder + arm1 + arm2) & chart_text).resolve_scale(x="shared")
```
...
fractions_long = (
  ...
  # reorder based on the dendro order
  .loc[dend_obj["ivl"]]
  .reset_index()
)

base_bar = alt.Chart(fractions_long, view=alt.ViewConfig(strokeWidth=0)).mark_bar().encode(
  x=alt.X("samplename:N").axis(grid=False),
  y=alt.Y("fractions:Q").axis(grid=False).scale(domain=[0,1]),
  color=alt.Color("celltype:N").legend(columns=2)
)
```

```{python}
(chart_den & base_bar).resolve_axis(x="shared")
```