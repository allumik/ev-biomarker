...
# check for the immune cell barcodes in the 
# np.unique(np.isin(sc_dat.obs.query("lineage == 'Immune'").index.to_numpy(), sc_dat_immu.obs.index.to_numpy()), return_counts=True)
# np.unique(np.isin(sc_dat_immu.obs.index.to_numpy(), sc_dat.obs.query("lineage == 'Immune'").index.to_numpy()), return_counts=True)
...
## also exclude cell types that have too few cells to begin with
## also exclude the endometriosis samples
count_tmp = sc_dat.obs.celltype.value_counts()
no_go_celltype = list(count_tmp[count_tmp < 50].index) + ['eHormones', 'sHormones', 'dHormones']
the_condition = "celltype not in @no_go_celltype & Endometrial_pathology == 'C'"
...
model_params = {
  ...
  "ratio_num": 0.8, # might solve some of the artifact problems
  "gpu": 0
}

frac_params = {
  "batch_size": 512,
  "epochs": 2500, # looking at loss plot then 500 seems to be already enough, and we don't need the fractions really
...
vae_params = {
  "batch_size": 512,
  "hidden_size": 256,
  "epoch_num": 2500 # looking at loss plot then 500 seems to be already enough
}
...
if gen_sc_file.exists() and not rerun_all:
  ...
  gen_ccht_ev.obs = gen_ccht_ev.obs.join(
    sc_dat.obs[["celltype", "lineage"]].drop_duplicates().set_index("celltype"), 
    on="celltype"
    )
...
ct_group = gen_ccht_ev.obs.lineage.unique()
for ct_g in ct_group:
  cts = gen_ccht_ev.obs.query("lineage==@ct_g").celltype.unique()
  sc.pl.spatial(
    vis_dat_proj,
    title=ct_g,
    cmap='magma',
...
# TODO multiclass colors for each major cell type group
ct_group = "Stromal"
cts = gen_ccht_bio.obs.query("lineage==@ct_group").celltype.unique()
sc.pl.spatial(
  vis_dat_proj,
  cmap='magma',
  color=cts,
...
# TODO multiclass colors for each major cell type group
ct_group = "Epithelial"
cts = gen_ccht_ev.obs.query("lineage==@ct_group").celltype.unique()
sc.pl.spatial(
  vis_dat_proj,
  cmap='magma',
  color=cts,
...
# TODO multiclass colors for each major cell type group
ct_group = "Stromal"
ct_group = gen_ccht_bio.obs.lineage.unique()[1]
cts = gen_ccht_bio.obs.query("lineage==@ct_group").celltype.unique()
sc.pl.spatial(
  vis_dat_proj,
  cmap='magma',
