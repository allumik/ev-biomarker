...
# load the object into memory for the double indexing that is going to happen
sc_dat = sc_dat.to_memory()

## also exclude the endometriosis samples
the_condition = "(celltype not in @sc_dat.var.index) & (Endometrial_pathology == 'C')"
sc_dat = sc_dat[sc_dat.obs.query(the_condition).index, :].copy() # have to copy to "deview" it
...
frac_params = {
  "batch_size": 512,
  "epochs": 50, # looking at loss plot then 500 seems to be already enough, and we don't need the fractions really
...
vae_params = {
  "batch_size": 512,
  "hidden_size": 256,
  "epoch_num": 50 # looking at loss plot then 500 seems to be already enough
}
