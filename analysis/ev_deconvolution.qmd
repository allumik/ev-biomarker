...
# create a colormap
lut = dict(zip(
  general_cells.lineage.unique(),
  sns.color_palette("Set2", general_cells.lineage.unique().size).as_hex()
  ))

col_colors = general_cells.lineage.astype("object").map(lut)
...
def calculate_stats(group):
  if len(group) == 2:
    ratio_ns = group.iloc[0]
    ratio_biopsy = group.iloc[1]
    correlation, p_value = spearmanr(ratio_ns, ratio_biopsy)
    mean_difference = np.abs(ratio_biopsy.mean() - ratio_ns.mean())
...
HTML(results_df.sort_values("HUT").to_html(index=False))

```

Then lets do a correlation matrix of paired samples with the EV samples on the Y axis and the biopsy samples on the X axis. We organise the samples in order with hierarchical clustering, so that more similar samples between sampling method are closer to each other.
...
corr_mat = (
  ...
  .reindex(ccht_uf_pheno.sort_values("cyclephase").index)
...
Finally, the heatmap of the difference of proportions per paired sample per celltype, showing only statistically signficiantly differing celltypes.
