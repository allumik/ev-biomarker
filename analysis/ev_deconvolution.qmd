...
Accessory markers PDCD6IP, SDCBP (syntenin-1), SDC4 (syndecan)

```{python}
gene_selection=["PDCD6IP", "SDCBP", "SDC4"]
plot_gene_list(sc_dat, gene_selection, wrap_rows=2)
```
...
sc_dat_tmp = ov.pp.preprocess(sc_dat, mode="shiftlog|pearson", n_HVGs=3000, target_sum=50*1e4)
gene_selection=["CHMP2A", "CHMP2B", "CHMP3", "CHMP4A", "CHMP4B", "CHMP4C", "CHMP6"]
plot_gene_list(sc_dat, gene_selection, wrap_rows=3)
del sc_dat_tmp
```
...
ttest_table = pd.DataFrame({
  celltype: {
    "z_stat": ttest_ind(dat[dat["implantation"]]["ratio"], dat[~dat["implantation"]]["ratio"]).statistic,
    "p-val": ttest_ind(dat[dat["implantation"]]["ratio"], dat[~dat["implantation"]]["ratio"]).pvalue,
  }
  for celltype, dat in frac_long.groupby("celltype"):
    dat[~dat["implantation"]]
}).T
# 
