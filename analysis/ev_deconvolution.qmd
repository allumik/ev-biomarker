...
groupies = ["cyclephase", "dataset"]
fractions = frac_pred.join(comb_uf_pheno[groupies]).groupby(groupies).mean()
fractions.columns.names = ["celltype"]

# Data Reshaping for Altair: Convert to long format
fractions = fractions.stack().reset_index()
cyclephase_order = ["pro", "pre", "rec", "post"]
fractions["cyclephase"] = pd.Categorical(fractions["cyclephase"], categories=cyclephase_order, ordered=True)
fractions = fractions.rename(columns={0: "fractions"}).reset_index(drop=True)

# Add lineage information for the celltype
fractions = fractions.merge(general_cells, on="celltype")

row_charts = []  # List to hold charts for each row (lineage)
is_first_row = True
global_color_scale = alt.Scale(domain=fractions.celltype) # Global color scheme

for lineage_val in fractions.lineage.unique():
  ...
  current_row_celltypes = fractions.query("lineage==@lineage_val").celltype.unique()

  for dataset_val in fractions.dataset.unique():
    # Filter data for the current lineage and dataset
    filtered_df = fractions.query("lineage==@lineage_val & dataset==@dataset_val")

    # Create a chart for this dataset and lineage
    current_chart = alt.Chart(filtered_df).mark_area().encode(
      x=alt.X(
        'cyclephase:O', 
        axis=alt.Axis(labelAngle=-45, grid=False, domain=False, title=None),
        sort=cyclephase_order
        ),
      y=alt.Y(
        'fractions:Q',
...
final_chart = alt.vconcat(*row_charts, spacing=0).resolve_scale( # Adjust vertical spacing as needed
    x='shared',
    y='independent',
