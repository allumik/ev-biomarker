...
## load in the EV data
comb_uf_mat = pd.read_feather(Path(data_folder).expanduser() / "comb_uf_tpm.feather").set_index("gene_id")
comb_uf_pheno = pd.read_table(Path(data_folder).expanduser() / "comb_uf_pheno.tsv").set_index("samplename")
comb_uf_raw = pd.read_feather(Path(data_folder).expanduser() / "comb_uf.feather").set_index("gene_id")
...
uf_bio_genes = pd.read_table(Path(data_folder).expanduser() / "uf_bio_genes.tsv").locus.to_list()

## load in the SCRATCH clinical bio data
scratch_mat = pd.read_feather(Path(data_folder).expanduser() / "star_mat_pheno.feather").set_index("sample")
...
annot = pd.read_csv(Path(data_folder).expanduser() / "annot_table.csv")
clin_uf_raw = (
  clin_uf_raw
...
# UF and scRNA-seq cell type link
...
## create a samples-level view of the obs
filt1 = ["Whole_Uterus"]
filt2 = ["dHormones", "eHormones", "sHormones"]
cycle_filt = ["EP", "ES", "MS", "LS"] # "LP"
sc_obs_tmp = gene_x_sample_view(
    sc_dat[
      sc_dat.obs
        .query("Biopsy_type not in @filt1 & cyclephase in @cycle_filt")
        .index
    ],
...
Then, lets look at the **UF+Bio geneset** profile in different cell types.
...
The same plot as before, but with major celltype proportions.

```{python}

frac_pred_gen = (
  frac_pred
  .groupby(general_cells["celltype_general"], axis=1)
  .sum()
)

dendro_barplot(frac_pred_gen, comb_uf_pheno)
...
pca_df = (
  pd.DataFrame(data=pca.fit_transform(frac_pred), columns=["PC1", "PC2"])
  .set_index(frac_pred.index)
...
sns.scatterplot(x='PC1', y='PC2', hue='cyclephase', style="group", data=pca_df, palette='Set1')

# Plot the loadings as a biplot
for i, feature in enumerate(frac_pred): 
...
plt.legend(title='Cycle Phase', loc="upper left", bbox_to_anchor=(1.05, 1))
plt.grid(False)
# Major grid only
...
dendro_barplot(
  frac_pred.loc[comb_uf_pheno.query("group == 'UF'").index],
  comb_uf_pheno,
  legend_phase_bbox=(1.05, -0.2),
  legend_class_bbox=(1.05, -0.2),
  legend_phase_ncol=comb_uf_pheno.cyclephase.unique().size,
...
frac_pred_gen = (
  frac_pred
  .loc[comb_uf_pheno.query("group == 'UF'").index]
  .groupby(general_cells["celltype_general"], axis=1)
  .sum()
)
...
pca = PCA(n_components=2)
pca_df = (
  pd.DataFrame(data=pca.fit_transform(frac_pred.loc[comb_uf_pheno.query("group == 'UF'").index]), columns=["PC1", "PC2"])
  .set_index(frac_pred.loc[comb_uf_pheno.query("group == 'UF'").index].index)
  .merge(comb_uf_pheno, left_index=True, right_index=True)
)
expl_var = pca.explained_variance_ratio_
loadings = pca.components_.T * np.sqrt(pca.explained_variance_)
...
# Visualize the PCA results
plt.figure(figsize=(8, 6))
sns.scatterplot(x='PC1', y='PC2', hue='cyclephase', data=pca_df, palette='Set1')
...
frac_long = convert_to_long(frac_pred.loc[comb_uf_pheno.query("group == 'UF'").index], comb_uf_pheno)
...
dendro_barplot(
  frac_pred.loc[comb_uf_pheno.query("group == 'biopsy'").index],
  comb_uf_pheno,
  legend_phase_bbox=(1.05, -0.2),
  legend_class_bbox=(1.05, -0.2),
  legend_phase_ncol=comb_uf_pheno.cyclephase.unique().size,
...
frac_pred_gen = (
  frac_pred
  .loc[comb_uf_pheno.query("group == 'biopsy'").index]
  .groupby(general_cells["celltype_general"], axis=1)
  .sum()
)
...
pca = PCA(n_components=2)
pca_df = (
  pd.DataFrame(data=pca.fit_transform(frac_pred.loc[comb_uf_pheno.query("group == 'biopsy'").index]), columns=["PC1", "PC2"])
  .set_index(frac_pred.loc[comb_uf_pheno.query("group == 'biopsy'").index].index)
  .merge(comb_uf_pheno, left_index=True, right_index=True)
)
expl_var = pca.explained_variance_ratio_
loadings = pca.components_.T * np.sqrt(pca.explained_variance_)
...
dendro_barplot(frac_pred_gen, ccht_uf_pheno)
...
dendro_barplot(
  frac_pred.loc[ccht_uf_pheno.query("group == 'UF'").index], 
  ccht_uf_pheno,
...
frac_pred_gen = (
  frac_pred
  .loc[ccht_uf_pheno.query("group == 'UF'").index]
  .groupby(general_cells["celltype_general"], axis=1)
  .sum()
)
...
pca = PCA(n_components=2)
pca_df = (
  pd.DataFrame(data=pca.fit_transform(frac_pred.loc[ccht_uf_pheno.query("group == 'UF'").index]), columns=["PC1", "PC2"])
  .set_index(frac_pred.loc[ccht_uf_pheno.query("group == 'UF'").index].index)
  .merge(ccht_uf_pheno, left_index=True, right_index=True)
)
expl_var = pca.explained_variance_ratio_
loadings = pca.components_.T * np.sqrt(pca.explained_variance_)
...
frac_long = convert_to_long(frac_pred.loc[ccht_uf_pheno.query("group == 'UF'").index], ccht_uf_pheno)
...
dendro_barplot(
  frac_pred.loc[ccht_uf_pheno.query("group == 'biopsy'").index], 
  ccht_uf_pheno,
  legend_phase_bbox=(1.05, .1),
  legend_class_bbox=(1.05, .1),
  legend_phase_ncol=comb_uf_pheno.cyclephase.unique().size,
...
frac_pred_gen = (
  frac_pred
  .loc[ccht_uf_pheno.query("group == 'biopsy'").index]
  .groupby(general_cells["celltype_general"], axis=1)
  .sum()
)
...
pca = PCA(n_components=2)
pca_df = (
  pd.DataFrame(data=pca.fit_transform(frac_pred.loc[ccht_uf_pheno.query("group == 'biopsy'").index]), columns=["PC1", "PC2"])
  .set_index(frac_pred.loc[ccht_uf_pheno.query("group == 'biopsy'").index].index)
  .merge(ccht_uf_pheno, left_index=True, right_index=True)
)
expl_var = pca.explained_variance_ratio_
loadings = pca.components_.T * np.sqrt(pca.explained_variance_)
...
dendro_barplot(frac_pred_gen, scratch_pheno, pheno_dat_col="no_lb")
...
# Visualize the PCA results
plt.figure(figsize=(8, 6))
sns.scatterplot(x='PC1', y='PC2', hue='status_rif', style="no_lb", data=pca_df, palette='flare')
...
plt.xlabel(f'PC1 ({expl_var[0]:.2%})')
plt.ylabel(f'PC2 ({expl_var[1]:.2%})')
plt.legend(title='Implantation', loc="upper left", bbox_to_anchor=(1.05, 1))
plt.grid(False)
# Major grid only
plt.axhline(0, color="lightgray", linewidth=1)
plt.axvline(0, color="lightgray", linewidth=1)
...
dendro_barplot(frac_pred_gen, clin_uf_pheno, pheno_dat_col="implantation")
...
# Visualize the PCA results
plt.figure(figsize=(8, 6))
sns.scatterplot(x='PC1', y='PC2', hue='implantation', style="n_transfer_embryo", data=pca_df, palette='Set1')
