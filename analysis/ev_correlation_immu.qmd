---
title: "Single cell perspective into Extracellular Vesicules on Immune cells"
format:
  html:
    code-fold: true
    code-tools: true
...
sc_dat = sc.read_h5ad(
  Path(anndata_folder).expanduser() / "endometriumAtlasV2_cells_immune.h5ad",
  backed = "r"
)
...
sc_dat.obs = (
  sc_dat.obs
  .assign(cyclephase=lambda x: x.Stage.replace(replace_dict))
)
...
How are cell types divided throughout the menstrual cycle?

```{python}

sns.displot(sc_dat.obs, x="Stage", hue="celltype", multiple="stack").set_xticklabels(rotation=90)
...
## switch the expression matrix to raw counts
...
fractions_folder = Path(data_folder) / "tape_fractions_immu"
...
# create a colormap
lut = dict(zip(
  sc_dat.obs.celltype.unique(),
  sns.color_palette("Set2", sc_dat.obs.celltype.unique().size).as_hex()
  ))

col_colors = sc_dat.obs.celltype.astype("object").map(lut)
